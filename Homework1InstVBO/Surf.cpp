...
GLuint create_strip_index_buffer(int n_grid)
{
    //Declare a vector to hold GL_POINTS vertices, texture coordinates, and normals 
    const int num_indices = (n_grid - 1) * (n_grid * 2 + 1);
    ...
    for (int i = 0; i < n_grid - 1; i++)
    {
        ...
        surf_indices.push_back(0xFFFFFFFF);
    }
...
surf_vao create_indexed_surf_interleaved_strip_vao(int n)
{
    surf_vao surf;
    ...
    surf.num_indices = (n - 1) * (n * 2 + 1);
    surf.type = GL_UNSIGNED_INT;

    GLuint index_buffer = create_strip_index_buffer(n);
